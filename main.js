...
var StageManager = /** @class */ (function () {
    ...
    StageManager.prototype.addGhost = function () {
        ...
        var ghost = new Ghost(this.svg, start, end);
...
function triggerGhosts() {
	var stageManager =  new StageManager(Snap('svg'));
	stageManager.init();
	makeGhost(stageManager);
};

function makeGhost(stageManager) {
	let intervalId =setInterval(function() {
		stageManager.addGhost()
  }, 250);

  setTimeout(function() {
    stopGhosts(intervalId);
}, 3000);
}

function stopGhosts(intervalId){
	console.log(intervalId);
	clearInterval(intervalId);
	$("#ghosts").css("display","none")
}